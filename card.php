...
if (isset($_GET['add_to_card'])) {
    ...
    try {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            header("Location: card.php"); // Sau khi thêm vào giỏ hàng, chuyển đến trang giỏ hàng
            exit();
        }
...
if (isset($_GET['update_quantity'])) {
    ...
    header("Location: card.php"); // Sau khi cập nhật, chuyển đến trang giỏ hàng
    exit();
}
...
if (isset($_GET['remove_from_card'])) {
    ...
    header("Location: card.php?message=removed");
    exit();
}
?>
<!DOCTYPE html>
        ...
        <?php else: ?>
            <table>
                ...
                <tr>
                    ...
                    <td>
                        <a href="card.php?update_quantity=1&product_id=<?php echo $product['id']; ?>&action=increase">+</a>
                        <?php echo $product['quantity']; ?>
                        <a href="card.php?update_quantity=1&product_id=<?php echo $product['id']; ?>&action=decrease">-</a>
                    </td>
            ...
            <div class="checkout">
                <button onclick="location.href='checkout.php'">Tiến hành đặt hàng</button>
            </div>
        <?php endif; ?>
    </div>
    ...
    <script>
        ...
        function removeFromCart(productId) {
            if (confirm("Bạn có chắc chắn muốn xóa sản phẩm này khỏi giỏ hàng?")) {
                window.location.href = "card.php?remove_from_card=" + productId;
            }
        }
